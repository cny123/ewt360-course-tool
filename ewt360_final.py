...
<section class="panel"><div class="title">账户</div><div class="body"><div class="field"><label>账号</label><input id="account" autocomplete="username" placeholder="请输入账号"></div><div class="field"><label>密码</label><input id="password" type="text" autocomplete="current-password" placeholder="请输入密码"></div><div class="grid"><button class="primary" id="login">账号登录</button><button id="tokenLogin">Token 登录</button></div><div class="field"><label>已有 Token</label><input id="token" spellcheck="false" placeholder="粘贴 Token"></div><div class="grid"><button id="scan">扫描课程</button><button id="logout">退出登录</button></div><div class="muted" id="accountInfo">登录信息只保存在当前进程内存。</div></div></section>
...
<section class="panel"><div class="title">课程列表 <span id="queue">0</span></div><div class="body"><div class="grid"><input id="search" placeholder="搜索课程名称"><select id="filter"><option value="all">全部状态</option><option value="todo">待处理</option><option value="done">已达标</option></select></div></div><div id="courses"><div class="body muted">登录后扫描课程。</div></div></section>
<section class="panel"><div class="title">运行日志</div><div class="log" id="logs">等待操作。</div></section></main>
<script>
const $=id=>document.getElementById(id),api=async(path,opt={})=>{const r=await fetch(path,{headers:{"Content-Type":"application/json"},...opt});const d=await r.json();if(!r.ok||!d.ok)throw Error(d.error||"请求失败");return d};let lessons=[],currentState={lessons:[],job:{},logs:[]};
...
function render(s){currentState=s;lessons=s.lessons||[];const done=lessons.filter(x=>x.finished||Number(x.percent||0)>=80).length;$("total").textContent=lessons.length;$("done").textContent=done;$("todo").textContent=lessons.length-done;$("queue").textContent=(s.job?.done||0)+"/"+(s.job?.total||0);$("job").textContent=s.job?.state||"待命";$("state").textContent=s.loggedIn?"已登录":"未登录";if(s.loggedIn)$("accountInfo").textContent="schoolId="+s.schoolId+" · userId="+s.userId;const query=$("search").value.trim().toLowerCase(),status=$("filter").value,shown=lessons.filter(x=>{const ok=x.finished||Number(x.percent||0)>=80;return (status==="all"||(status==="done"&&ok)||(status==="todo"&&!ok))&&(!query||String(x.title||"").toLowerCase().includes(query))});const box=$("courses");box.innerHTML=shown.length?shown.map(x=>{const p=Math.max(0,Math.min(100,Number(x.percent||0))),ok=x.finished||p>=80;return '<article class="course '+(ok?'done':'')+'"><div class="course-head"><div><div class="course-title">'+esc(x.title||x.id)+'</div><div class="id">'+esc(x.subject||"")+' · lessonId='+esc(x.id)+'</div></div><span class="pill '+(ok?'done':'todo')+'">'+(ok?'已达标':'待处理')+'</span></div><div class="ptext"><span>'+p.toFixed(1)+'%</span><span>目标 80%</span></div><div class="bar"><span style="width:'+p+'%"></span></div></article>'}).join(""):'<div class="body muted">没有符合条件的课程。</div>';if(s.logs?.length){$("logs").innerHTML="";s.logs.slice(-60).forEach(x=>writeLog(x.message,x.level==="ERR"||x.level==="WARN"?"warn":"ok"))}}
...
$("tokenLogin").onclick=async()=>{try{const token=$("token").value.trim();if(!token)throw Error("请输入 Token");render((await api("/api/login",{method:"POST",body:JSON.stringify({token})})).state);writeLog("Token 登录成功","ok")}catch(e){writeLog(e.message,"warn")}};
...
$("logout").onclick=async()=>{try{render((await api("/api/logout",{method:"POST",body:"{}"})).state);$("accountInfo").textContent="已退出当前会话";writeLog("已退出登录","ok")}catch(e){writeLog(e.message,"warn")}};$("search").oninput=()=>render(currentState);$("filter").onchange=()=>render(currentState);
</script></body></html>'''
